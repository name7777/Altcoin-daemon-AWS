Bonocoin Core version *0.15.1* is now available from:

  <https://download.bonocoin.org/bonocoin-0.15.1/>
...
A number of changes to the way Bonocoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 s per kB (0.00001 BNC). This is to prepare for dropping the minimum transaction fee to 0.1 s per kB (0.0001 BNC) in the next release.
