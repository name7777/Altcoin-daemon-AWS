...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Bonocoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Bonocoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Bonocoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Bonocoin 0.7 datadir."
    ;;
  3)
    echo "Detected Bonocoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Bonocoin 0.8 datadir."
    ;;
esac
